...
public enum ExpressionParserError implements ErrorMessage {
  ...
  EXPECTED_RIGHT_INFIX_OPERAND("Expected a right-hand-side for this infix-operator: %s"),
  EXPECTED_PREFIX_OPERAND("Expected a right-hand-side for this prefix-operator: %s"),
  EXPECTED_BRANCH_DELIMITER("Expected the branching-delimiter colon: :"),
  ...
  EXPECTED_PARENTHESES_CONTENT("Expected the parentheses to bear some content: (...)"),
  EXPECTED_CLOSING_PARENTHESIS("Expected a closing-parenthesis after opening one: (...)"),
  EXPECTED_MEMBER_ACCESS_IDENTIFIER_RHS("The right-hand-side of a member-access operation may only be an identifier: %s.<identifier>"),
  ;
