...
public enum MarkupParseError {
  ...
  MULTIPLE_ATTRIBUTE_INTRINSIC_MARKERS(args -> "The asterisk-/plus-operator used to mark intrinsic attributes may only be used once!"),
  UNSUPPORTED_BINDING_NAME_EXPRESSION(args -> "The bind operator [...] on flag-attributes only supports either identifiers, or member-access-chains of identifiers"),
  COMBINED_INTRINSIC_MARKER_OPERATOR(args -> "The asterisk-/plus-operator used to mark intrinsic attributes cannot be combined with any other attribute-operators!"),
  ...
  CONTAINER_HAS_BOUND_FLAG_ATTRIBUTES_WHEN_NON_SELF_CLOSING(args -> "The container-tag only accepts children via bound flag-attributes when self-closing: <container />"),
  CONTAINER_MISSING_BOUND_FLAG_ATTRIBUTES_WHEN_SELF_CLOSING(args -> "The container-tag may only be self-closing if there are bound flag-attributes present"),
