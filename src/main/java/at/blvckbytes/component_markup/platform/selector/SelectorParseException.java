...
public class SelectorParseException extends RuntimeException implements ErrorMessage {
  ...
  public SelectorParseException(InputView input, SelectorParseError error, String... messagePlaceholders) {
    this(input, Math.max(0, input.getPosition()), error, messagePlaceholders);
  }

  public SelectorParseException(InputView input, int position, SelectorParseError error, String... messagePlaceholders) {
    this.input = input;
